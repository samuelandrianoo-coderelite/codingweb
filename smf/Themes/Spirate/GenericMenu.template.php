...
function template_generic_menu_dropdown_above()
{
	...
	echo'
	<a id="menuMounstro" href="#" class="position-fixed shadow d-md-none d-block btn_more"><span class="main_icons plus h1 "></span></a>';
	// Load the menu
	// Add mobile menu as well
	echo '
	<div class="row"> 	 
	<div class="offcanvas offcanvas-start p-0" data-bs-scroll="true" tabindex="-1" id="mobileMenu"> 
...
function template_generic_menu(&$menu_context)
{
	...
	echo '
				<div class="generic_menu">
					<div  id="spirit_nav_', $context['cur_menu_id'], '" class="accordion">';
	...
	{ 
		...
		<div id="',$key,'" class="accordion-collapse collapse  ', !empty($section['selected']) ? 'show ' : '', '" aria-labelledby="heading-',$key,'" data-bs-parent="#spirit_nav_', $context['cur_menu_id'], '">';
		...
		$oneLevel = [];
		echo'<div class="accordion accordion-flus" id="child-'.$key.'">
	';
		foreach ($section['areas'] as $i => $area)
		{
			...
			$oneLevel[]= '<a class="'.(!empty($area['selected'])? 'bg-primary text-body':'').' d-block px-4 py-2" href="'. (isset($area['url']) ? $area['url'] : $menu_context['base_url'] . ';area=' . $i). ''. $menu_context['extra_parameters']. '"  >
			...
			if (!empty($area['subsections']) && empty($area['hide_subsections']))
			{
				...
				<div class="accordion-item"> 
					...
					', $area['icon'], $area['label'], !empty($area['amt']) ? ' <span class="amt">' . $area['amt'] . '</span>' : '', '
					</button>
			    	</h3>

				<div id="flush-'.$i.'" class="accordion-collapse collapse ',!empty($area['selected'])? 'show':'','" aria-labelledby="flush-heading-'.$i.'" data-bs-parent="#child-'.$key.'">';
				foreach ($area['subsections'] as $sa => $sub)
				{
					...
					echo ' <a class="d-block px-4 py-2 text-muted ', !empty($sub['selected']) ? 'bg-primary text-body ' : '', '"  href="', $url, $menu_context['extra_parameters'], '">', $sub['label'], !empty($sub['amt']) ? ' <span class="amt">' . $sub['amt'] . '</span>' : '', '</a> 
										';
				} 
				echo'</div>
		     	</div>';
