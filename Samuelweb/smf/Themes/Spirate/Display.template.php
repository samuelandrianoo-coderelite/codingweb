	...
	//	
		...
		{ 
			...
			<div class="bg-secondary align-items-center smflikebutton p-2  text-center ',$is_comment?'d-md-flex':'d-md-block','" id="msg_', $message['id'], '_likes">
				...
				<a ',$guest ? ' href="'.$scripturl.'?action=login" onclick="return reqOverlayDiv(this.href,' . JavaScriptEscape($txt['login']) . ', \'login\');"':   (!$is_autor ? 'href="'.$likeHref.'" class="msg_like"':'') ,'>
				<span class="main_icons ',!$is_comment?'trending-up':'like','"></span>
				</a>
				<span class="counter d-block">',$message['likes']['count'],'</span>  
