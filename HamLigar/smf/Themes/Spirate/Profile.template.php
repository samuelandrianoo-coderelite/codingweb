...
function template_profile_popup()
{
	...
	foreach ($context['profile_items'] as $item)
	{
		...
		echo ' 
					<a href="', $item_url, '" class="p-2 d-block"><span class="h2 text-muted align-middle me-1">', $area['icon'], '</span>', !empty($item['title']) ? $item['title'] : $area['label'], '</a>
				 ';
	}
...
function template_alerts_popup()
{
	...
	echo '<div class="w-100">
		<div>
			<div class="alerts_opts block px-3">
	...
	else
	{
		foreach ($context['unread_alerts'] as $id_alert => $details)
		{
			echo '
			<hr class="m-0">
			<', !$details['show_links'] ? 'a href="' . $scripturl . '?action=profile;area=showalerts;alert=' . $id_alert . '" onclick="this.classList.add(\'alert_read\')"' : 'div', '>
			 <div class="d-flex w-100 p-2">
				...
				', $details['icon'], ' ', $details['text'], '<small class="d-block text-muted"> ', $details['time'], ' </small>
				</div> 
			</div>
...
function template_alerts_all_read()
{
	...
	echo '<hr class="my-1"><div class="no_unread px-3"> ', $txt['alerts_no_unread'], '</div>';
}
...
function template_summary()
{
   ...
   // var_dump($context['print_custom_fields']);
...
if (!empty($context['print_custom_fields']['standard']))
{
	...
	foreach ($context['print_custom_fields']['standard'] as $field){
		if($field['name'] == 'cover'){
			$cover = !empty($field['value'])? 	$field['value'] : '' . $settings['images_url'] . '/covers/default.svg';
		}
	} 
}
	...
	<div class="card w-100">
		<div class="cover bg-cover " style="height:300px;background-image: url(',$cover,')">
		</div>
		<div class="ms-2 d-flex align-items-end" style="margin-top: -4rem;">
		',template_avatar( $context['member']['avatar']['image'],'125','rounded-circle'),'
		  
			<div class="d-flex ms-3">'; 
			if (!isset($context['disabled_fields']['posts']))
			echo '
			<div class="p-2">', $context['member']['posts'], '<div class="text-muted small">', $txt['profile_posts'], ' </div></div>
			<div class="vr"></div>';

			echo'
			<div  class="p-2">', (!empty($context['member']['group']) ? $context['member']['group'] : $context['member']['post_group']), '<small class="d-block text-muted">',$txt['group'],'</small></div>		
			</div> 
		</div>
		<div class="px-3">
		<div class="py-2">
		 <strong>@', $context['member']['name'], '</strong>  <small class="text-muted">', $txt['since_registered'], ' ', $context['member']['registered'], '</small>
		</div>
			...
			{ 
				foreach ($context['print_custom_fields']['icons'] as $field)
					if (!empty($field['output_html']))
						echo '
						<span class="btn_more bg-secondary">', $field['output_html'], '</span>';
			}
		...
		echo '
			<div class="signature px-3 ">
				...
				<blockquote class="blockquote">
					<p>', $context['member']['signature'], '.</p>
				</blockquote>
				<figcaption class="blockquote-footer">
				', $txt['signature'], ' <cite title="', $context['member']['name'], '">', $context['member']['name'], '</cite>
				</figcaption>
				</figure>
			</div>';
		echo'
		<div class="p-1 p-md-3"> 
			<a class="d-block mb-2 d-md-inline-block btn btn-primary" href="', $scripturl, '?action=profile;area=showposts;u=', $context['id_member'], '" >', $txt['showPosts'], '</a>';

	if ($context['user']['is_owner'] && !empty($modSettings['drafts_post_enabled']))
		echo '
			<a class="d-block mb-2 d-md-inline-block btn btn-primary" href="', $scripturl, '?action=profile;area=showdrafts;u=', $context['id_member'], '" >', $txt['drafts_show'], '</a>';

	echo '
			<a class="d-block mb-2 d-md-inline-block btn btn-primary" href="', $scripturl, '?action=profile;area=statistics;u=', $context['id_member'], '" >', $txt['statPanel'], '</a>
